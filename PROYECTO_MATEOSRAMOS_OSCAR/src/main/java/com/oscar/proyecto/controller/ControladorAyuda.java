...
public class ControladorAyuda implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        webEngine = webViewAyuda.getEngine();
        listaTemas.getItems().addAll(ayudas.keySet());
        listaTemas.getSelectionModel().selectedItemProperty().addListener(
            (obs, oldVal, newVal) -> {
                if (newVal != null) {
                    temaActual = newVal;
    ...
    private void mostrarContenido(String tema) {
        String contenidoHTML = generarHTML(tema);
        webEngine.loadContent(contenidoHTML);
    }

    private String generarHTML(String tema) {
        String titulo = "Ayuda: " + tema;
        String texto = ayudas.getOrDefault(tema, "No hay ayuda disponible para este tema.");
    ...
    private void cerrarAyuda() {
        webViewAyuda.getScene().getWindow().hide();
    }
