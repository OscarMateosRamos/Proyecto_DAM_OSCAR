...
public class ControladorGestionEstudiantes {
    ...
    private void cargarEstudiantes() {
        listaEstudiantes = FXCollections.observableArrayList(estudianteServicio.listarEstudiantes());
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == guardarButtonType) {
                ...
                if (!validarDni(dni.getText())) {
                    mostrarError("DNI inválido", "El DNI debe tener 8 números y una letra.");
                    return null;
                }

                if (!validarEmail(email.getText())) {
                    mostrarError("Email inválido", "Introduce un email válido.");
                    return null;
                }
                ...
                if (fechaNac.getValue() == null) {
                    mostrarError("Fecha requerida", "Debes seleccionar una fecha de nacimiento.");
                    return null;
                }
    ...
    private void editarEstudiante() {
        ...
        if (seleccionado == null) {
            mostrarError("Selecciona un estudiante", "Debes seleccionar un estudiante para editarlo.");
            return;
        }
        ...
        Label lblPersona = new Label(seleccionado.getNombre() + " " + seleccionado.getApellidos());
    ...
    private void eliminarEstudiante() {
        ...
        if (seleccionado == null) {
            mostrarError("Selecciona un estudiante", "Debes seleccionar un estudiante para eliminarlo.");
            return;
        }

        Alert confirmacion = new Alert(Alert.AlertType.CONFIRMATION);
        confirmacion.setTitle("Confirmar eliminación");
        confirmacion.setHeaderText("¿Seguro que deseas eliminar al estudiante?");
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try {
                estudianteServicio.eliminarEstudiante(seleccionado.getIdPersona());
